...
import { expectCodeError } from "./helpers";
...
describe("zip", () => {
    ...
    it("zip broken symlink (followSymlinks = false)", async () => {
        ...
        expect(stat1.isSymbolicLink(), `${path.join(des, "symlink")} is not a symlink`).toBe(true);
        expect(stat2.isSymbolicLink(), `${path.join(des, "subfolder_symlink")} is not a symlink`).toBe(true);
        expect(stat3.isSymbolicLink(), `${path.join(des, "broken_file_symlink")} is not a symlink`).toBe(true);
    });
    ...
    it("zip broken symlink (followSymlinks = true)", async () => {
        ...
        await expectCodeError(
            () =>
                zl.archiveFolder(path.join(__dirname, "../resources_with_broken_symlink"), zipFile, {
                    followSymlinks: true,
                }),
            "ENOENT",
            "zip broken symlink (followSymlinks = true)",
        );
    });
});
