...
describe("unzip", () => {
    ...
    it(
        ...
        async () => {
            ...
            await expect(unzip.extract(zipBuffer, path.join(__dirname, "../unzips/node_modules_from_buffer"))).rejects.toMatchObject({
                name: "Canceled",
            });
        },
        10000,
    ...
    it(
        "cancel extract zip file and try again",
        async () => {
            const unzip = new zl.Unzip({
                overwrite: true,
            });
    ...
    it(
        "cancel extract from buffer and try again",
        async () => {
            const unzip = new zl.Unzip({
                overwrite: true,
            });
